 ...
 *
 * Interview note:
 *   - STATELESS session policy means no server-side sessions
 *   - Each request must carry a valid JWT
 *   - CORS allows React frontend (localhost:5173) to call the API
 */
@Configuration
@EnableWebSecurity
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/api/transfers/**").hasAnyRole("ADMIN", "WAREHOUSE_MANAGER")
